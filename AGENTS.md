...
### 2026-08-11 (20)
- Did: Fixed a real navbar bug, not a style preference — `Navbar.tsx`'s text-links breakpoint was `md:flex` (768px), so windows between ~640-767px collapsed into the mobile hamburger even though the full "Product Privacy Contact" + Download layout fits comfortably at that width, and no mockup ever shows a hamburger. Fahim's 653px screenshot exactly demonstrated the bug. Lowered the breakpoint from `md` to `sm` (640px) across all three spots (text links, hamburger button, mobile menu panel). Confirmed desktop (1440px) was always correct — Fahim's own `/privacy` full-width screenshot verified it — this was purely a mid-width regression window. Verified with build + Puppeteer screenshots at 653px (fixed), 640px (fits cleanly at the new edge), and 390px (hamburger still correctly appears on a true phone width).
